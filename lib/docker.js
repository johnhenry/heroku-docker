...
function runImage(imageId, cwd, command, mount, ports) {
  ports = ports || {};
  if (!imageId) return;
  ...
  var runCommand = `run -p ${ports.to || 3000}:${ports.from || 3000} --rm -it ${mountComponent} ${envArgComponent} ${imageId} sh -c "${commandWithEnv}" || true`;
